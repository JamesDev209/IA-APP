...
const ProjectCard = ({ gen, forCommunity = false }:
    ...
    const handleDelete = async (id: string) => {
        const confirm = window.confirm('Are you sure you want to delete this proyect?')
        if (!confirm) return;
        console.log(id)
    }

    const toglePublish = async (projectId: string) => {
                ...
                <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
                    {gen.generatedImage && (
                        <img
                            ...
                            alt={gen.productName}
                            className={`absolute insert-0 w-full h-full object-cover transition duration-500 ${gen.generatedVideo ? 'group-hover:opacity-0' : 'group-hover:scale-105'}`}
                        />
                    )}

                    {gen.generatedVideo && (
                        <video
                            src={gen.generatedVideo} muted loop playsInline
                            className={`absolute insert-0 w-full h-full object-cover opacity-0 transition duration-500 group-hover:opacity-100`}
                    ...
                    {gen.isGenerating && (
                        <span className="text-xs px-2 py-1 bg-yellow-600/30 rounded-full"> Genarating</span>
                    )}
                    ...
                    {!forCommunity && (
                        <div
                            ...
                            className="absolute right-3 top-3 flex sm:opacity-0 group-hover:opacity-100 gap-2 items-center transition ">
                            ...
                            <div className="flex flex-col items-end w-32 text-sm ">
                                ...
                                overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                                    ...
                                    {(gen.generatedImage || gen.generatedImage) &&
                    ...
                    <div className="absolute right-3 bottom-3">
                        <img src={gen.uploadImages[0]} alt="Product" className="w-16 h-16 object-cover rounded-full animate-float" />
                        <img src={gen.uploadImages[1]} alt="Model" className="w-16 h-16 object-cover rounded-full animate-float -ml-8"
                ...
                <div className="p-4">
                    ...
                    {!forCommunity && (
                        <div className="mt-4 grid grid-cols-2 gap-3">
                            <GhostButton  className="" onClick={()=>{navigate(`/result/${gen.id}`);scrollTo(0,0);}}>
                                View Details 
                            </GhostButton>

                            <PrimaryButton onClick={()=> toglePublish(gen.id)} className="rounded-md">
